...
% === Scientists ===
\node[scientist, minimum width=3.0cm] (dN) at (1.6, 0) {
  \textbf{denario-1..\textit{N}}\\[1pt]
  {\scriptsize OpenClaw + Slack}\\[-1pt]
  {\scriptsize Denario pipeline}\\[-1pt]
  {\scriptsize\itshape Supervised fleet}
};
\node[scientist] (da) at (5.6, 0) {
  \textbf{Denario}\\[1pt]
  {\scriptsize Python loop}\\[-1pt]
  {\scriptsize Denario pipeline}\\[-1pt]
  {\scriptsize\itshape Autonomous fleet}
};
\node[scientist] (ce) at (9.0, 0) {
  \textbf{CosmoEvolve}\\[1pt]
  {\scriptsize OpenClaw}\\[-1pt]
  {\scriptsize CosmoEvolve pipeline}\\[-1pt]
  {\scriptsize\itshape Virtual Lab}
};


% === Human supervisors (top left) ===
\node[service, minimum width=2.8cm] (human) at (1.6, 3.5) {Human Supervisors\\[-1pt]{\scriptsize Slack}};

% === Mission Control (top right) ===
\node[service, minimum width=2.6cm] (dash) at (7.3, 3.5) {Mission Control\\[-1pt]{\scriptsize Fleet dashboard}};
...
\node[box, fill=ForestGreen!15, text=ForestGreen!80!black] (prv) at (8.4, -3.6) {\textbf{Parallel Open Review}\\[-1pt]{\scriptsize reviews.parallelscience}};
...
% LLMs
\draw[darr] (llms.east) -- (dN.west);

% Scientists -> GitHub
\draw[arr] (dN.south) -- ++(0,-1.8) -| ([xshift=-4pt]gh.north);
\draw[arr] (da.south) -- ++(0,-2.0) -| ([xshift=4pt]gh.north);
\draw[arr] (ce.south) -- ++(0,-1.8) -| ([xshift=12pt]gh.north);
